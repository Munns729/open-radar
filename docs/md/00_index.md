# RADAR Developer Documentation

Welcome to the technical documentation for **RADAR (Real-time Automated Discovery & Analysis for Returns)**.

This documentation is designed to help developers and AI agents understand the system's architecture, modules, and workflows.

## Table of Contents

### 1. System Overview
- **[01_architecture.md](./01_architecture.md)**: High-level system architecture, tech stack, and data flow.

### 2. Core Components
- **[02_core.md](./02_core.md)**: Configuration, Database, and base classes.

### 3. Functional Modules
- **[03_universe.md](./03_universe.md)**: Universe Scanner (Discovery, Enrichment, Moat Scoring).
- **[04_intelligence.md](./04_intelligence.md)**: Deal Intelligence (Valuation, Market Intel).
- **[05_capital.md](./05_capital.md)**: Capital Flows (PE Firms, Portfolios).
- **[thesis_validator.md](./thesis_validator.md)**: Thesis Validator (Company fit, Leaderboard).
- **[06_competitive.md](./06_competitive.md)**: Competitive Radar (VC Tracking, Threats).
- **[07_carveout.md](./07_carveout.md)**: Carveout Scanner (Corporate Parents, Signals).
- **[08_relationships_tracker.md](./08_relationships_tracker.md)**: Relationship Manager & Target Tracker.

### 4. Interface & Operations
- **[09_web_api.md](./09_web_api.md)**: Web Application & API Reference.
- **[10_workflows.md](./10_workflows.md)**: Common Workflows, Scripts, and Debugging.

## Quick Links
- [Project README](../../README.md)
- [Agent Capabilities](../../AGENTS.md)
