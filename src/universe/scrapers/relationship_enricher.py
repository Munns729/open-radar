...
class RelationshipEnricher:
    ...
    async def _search_for_contracts(self, page: Page, company_name: str) -> List[Dict[str, Any]]:
        ...
        for q in queries:
            try:
                # Search using search engine (DuckDuckGo or Google)
                # Using DDG HTML for stability
                await page.goto(f"https://html.duckduckgo.com/html/?q={q}", timeout=15000)
                
                results = await page.locator(".result__body").all()
                for res in results[:5]: # Top 5
                    text = await res.inner_text()
                    # Heuristic NLP Extraction
                    # "X selected by Y to provide Z"
                    # "X awarded contract by Y"
                    
                    if "selected by" in text.lower():
                        tgt = text.lower().split("selected by")[1].split(" to ")[0].strip()
                        if len(tgt) < 50: # valid length
                             found.append({
